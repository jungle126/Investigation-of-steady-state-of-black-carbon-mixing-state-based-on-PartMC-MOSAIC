...
fig, ax = plt.subplots(figsize=(3,3.25),constrained_layout=False)
...
ax.set_ylabel( r'ln(n($\mathrm{D_p}$))', fontsize=10,labelpad = 0)
ax.set_xlabel(r'$\mathrm{D_p}$(nm)', fontsize=10,labelpad = 2)
ax.text(520,-3,'k = '+str(abs(round(k,3))),fontsize=9,horizontalalignment='center', verticalalignment='center')
#ax.text(520,-4,'$\mathrm{R^2}$ = '+str(abs(round(R_2,3))),fontsize=10,horizontalalignment='center', verticalalignment='center')

ax.set_xlim([140,600])
ax.set_ylim([-13,-3])
ax.set_yticks([-12,-10,-8,-6,-4,-2])
